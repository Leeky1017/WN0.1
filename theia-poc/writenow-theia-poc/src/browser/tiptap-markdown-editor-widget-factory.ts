...
import {
  TIPTAP_MARKDOWN_EDITOR_WIDGET_FACTORY_ID,
  TipTapMarkdownEditorWidget,
} from './tiptap-markdown-editor-widget';

/**
 * Why: `WidgetOpenHandler` expects a widget factory registered under the same `id`.
...
export class TipTapMarkdownEditorWidgetFactory implements WidgetFactory {
  ...
  async createWidget(options: NavigatableWidgetOptions): Promise<TipTapMarkdownEditorWidget> {
    ...
    const widget = new TipTapMarkdownEditorWidget(this.fileService, this.commandService);
