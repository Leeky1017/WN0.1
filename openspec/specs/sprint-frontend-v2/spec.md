...
### Requirement: MUST 迁移 TipTap 编辑器并实现多标签/分屏

团队 MUST 复用现有 TipTap 编辑器组件（Markdown 所见即所得），并新增多标签编辑、分屏视图、预览模式功能。

#### Scenario: 编辑器核心功能

- **WHEN** 用户打开 `.md` 文件
- **THEN** 必须用 TipTap 编辑器打开，支持 Markdown 所见即所得、浮动工具栏、斜杠命令、块级拖拽、代码块语法高亮
