...
- Add: `src/components/AI/ContextDebugPanel.tsx`
- Add: `src/types/context-debug.ts`
- Update: `src/components/AI/DiffView.tsx`（在 AI 面板中接入 viewer 入口）
- Update: `src/components/AIPanel.tsx`
- Add: `tests/e2e/sprint-2.5-context-engineering-context-viewer.spec.ts`

## Acceptance Criteria

- [x] ContextViewer 支持按层级分区展示（Rules/Settings/Retrieved/Immediate）
- [x] 展示 per-layer used/budget + total used/limit，并能显示裁剪摘要（removed/compressed）
- [x] 每个 chunk 展示来源（source）与 tokenCount；支持复制完整 prompt
- [x] 对敏感信息（API key 等）必须脱敏显示

## Tests

- [x] Playwright E2E：触发上下文预览 → 展开 viewer → 断言分层、token 统计、来源可见；验证脱敏规则生效
