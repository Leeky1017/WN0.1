...
- [ ] 稳定前缀中不得出现动态元素（时间戳/随机数/不稳定排序）
- [ ] Rules 的渲染顺序固定且可追溯（按文件名排序，并标注来源文件）
- [ ] 动态后缀严格按 Settings → Retrieved → Immediate 的顺序注入
- [ ] PromptTemplate 具备版本字段（便于未来迭代不破坏回溯）

## Tests

- [ ] Vitest：同输入多次渲染稳定前缀完全一致；Rules 文件顺序稳定
- [ ] Playwright E2E：连续两次触发上下文预览 → 断言稳定前缀一致（可通过 UI 提供 prefix hash 证据）
