...
- [x] 选区变化、光标移动、段落变更能够在 200ms（默认）内同步到 Store（可配置）
- [x] 同步不会导致编辑器卡顿；高频光标移动不产生明显掉帧
- [x] Immediate 层至少能提供：selectedText、currentParagraph、before/after N 段（N 可配置）

## Tests

- [x] Playwright E2E：输入多段文本 → 选中/移动光标 → 触发上下文预览 → 断言 Immediate 内容与 UI 中所见一致
