...
|------:|------|------|--------|----------|-----------------|-------------------|
| 0 | 系统指令/不变量 | 内置常量 + `AGENTS.md` 的关键约束摘录（由 renderer 组装进稳定模板） | N/A | 极低 | 800–1500 | Markdown 固定章节 |
...
## 与 Sprint 2.5（Context Engineering）四层模型映射（避免术语漂移）

历史上 WriteNow 使用过 4 层上下文模型（Rules/Settings/Retrieved/Immediate）。为保持一致性与可迁移性：
- **6 层模型是本 Sprint 的 SSOT**（用于来源、预算、稳定性约束）
- **4 层模型可作为 UI/调试视图的聚合展示**（把 6 层投影到 4 层）

| Sprint 2.5 四层 | 含义 | 主要来源（对应 6 层） |
|----------------|------|------------------------|
| Rules | 始终加载的硬约束/规则 | Layer 0（系统不变量）+ Layer 2（用户偏好规则）+ Layer 3（项目规则/风格指南中的“规则部分”） |
| Settings | 项目设定（人物/世界观/大纲） | Layer 3（项目设定） |
| Retrieved | 动态检索/摘要 | Layer 4（最近摘要）+ Layer 5（RAG/局部检索结果，如有） |
| Immediate | 实时编辑器上下文/用户指令 | Layer 1（会话/编辑器状态）+ Layer 5（选区/前后文/当前任务） |
