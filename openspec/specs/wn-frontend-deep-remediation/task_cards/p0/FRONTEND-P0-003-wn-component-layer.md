...
- [ ] `Wn*` 组件 props 命名一致，且默认密度/圆角/阴影来自 tokens（禁止重复定义）
- [ ] Sidebar/AIPanel 至少完成第一批迁移：移除显著的硬编码颜色与重复布局代码
- [ ] 组件具备可访问性语义（role/label），便于 E2E 稳定定位

## Tests

- [ ] 更新/新增 Playwright E2E，覆盖 Sidebar/AIPanel 在迁移后仍可完成核心交互（打开文档、发送消息等）
