...
- [x] 顶部与底部不再重复展示字数/保存状态等信息
- [x] StatusBar 高度 ≤ 24px，信息分区清晰（左：文件/保存；中：编辑信息；右：辅助功能）
- [x] 番茄钟不再“审美入侵”：默认不占用固定全局横条，仅以微型进度呈现（可展开详情）

## Tests

- [x] Playwright E2E：写作 → 自动保存状态可见；切换番茄钟 → 微型进度可见且可展开
