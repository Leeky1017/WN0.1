...
- [x] PoC 能明确回答：File Explorer 展示什么路径？项目 root 在哪里？`.writenow` 与 DB 在哪里？
- [x] 选定方案对 E2E 的影响被明确：如何隔离 userData/workspace、如何创建临时项目、如何验收真实持久化。
- [x] 选定方案对索引器/watcher 的影响被明确：监听根目录、忽略规则、增量索引策略。
- [x] 形成正式决策记录（写入 `design/storage-model.md` 或等价位置），并标明“未来变更需再次 PoC/为何不支持双栈”。
