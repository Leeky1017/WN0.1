...
RUN_LOG: openspec/_ops/task_runs/ISSUE-141.md
...
- [x] frontend 可通过 RPC 调用 backend handler，并获得稳定的 `IpcResponse<T>` 结果（ok/err 可判定）。
- [x] 合约漂移可被检测（本地/CI）：channel 集合与类型不一致会失败。
- [x] 至少两条真实能力链路在 Theia 上可用（例如：创建项目/读写文件），并有可复现的验证步骤。
