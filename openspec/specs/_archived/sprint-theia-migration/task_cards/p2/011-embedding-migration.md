...
- [x] Theia backend 可成功生成 embedding（对一段文本返回向量），并具备超时/取消的稳定语义（`TIMEOUT`/`CANCELED`）。
- [x] ONNX 资产兼容性验证有明确证据（记录平台/CPU/结果），不通过时有明确错误信息与后续动作。
- [x] Embedding 失败不会导致系统卡死：pending 状态清理，用户可重试或降级。
