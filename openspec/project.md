# WriteNow

AI 驱动的文字创作 IDE

## 规范索引

- [writenow-spec](specs/writenow-spec/spec.md): 产品规范（愿景/核心概念/系统架构/路线图）
- [sprint-theia-migration](specs/sprint-theia-migration/spec.md): Theia 迁移（主线）
- [api-contract](specs/api-contract/spec.md): IPC/RPC 契约规范（SSOT）
- [sprint-1-editor](specs/sprint-1-editor/spec.md): 编辑器闭环
- [sprint-2-ai](specs/sprint-2-ai/spec.md): AI 基础链路 + Judge layer
- [sprint-2.5-context-engineering](specs/sprint-2.5-context-engineering/spec.md): 上下文工程
- [sprint-3-rag](specs/sprint-3-rag/spec.md): RAG / 搜索
- [sprint-4-release](specs/sprint-4-release/spec.md): 发布准备
- [sprint-5-project](specs/sprint-5-project/spec.md): 项目管理
- [sprint-6-experience](specs/sprint-6-experience/spec.md): 体验增强（部分暂停）
- [sprint-7-cloud](specs/sprint-7-cloud/spec.md): 云服务
- [skill-system-v2](specs/skill-system-v2/spec.md): SKILL 系统 V2（任务 004–010 暂停）
- [wn-frontend-deep-remediation](specs/wn-frontend-deep-remediation/spec.md): 前端深度治理（Paused）
- [sprint-ide-advanced](specs/sprint-ide-advanced/spec.md): IDE 高级能力（Paused）
