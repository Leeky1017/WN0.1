...
- **禁止替换已锁定的技术选型**：Sprint 规范中标记为"锁定"的技术（如 TipTap、FlexLayout、shadcn/ui 等）禁止替换为其他方案
- 如需变更技术选型，必须先提交 RFC 并获得批准
- 未经批准的技术替换视为**任务失败**，代码不予合并

### 设计文档必读

执行任何工程任务前，**必须先阅读相关设计文档**：

1. 查看 Sprint 规范的"技术栈锁定"章节
2. 阅读 `design/` 目录下的相关设计文档
3. 阅读任务卡片中的"必读前置"字段列出的文档
4. 确认使用锁定的技术栈后再开始编码

### 任务卡片执行顺序

1. **读取任务卡片** → 确认任务范围
2. **阅读必读前置文档** → 确认技术选型和设计约束
3. **创建 RUN_LOG** → 记录执行过程
4. **编码实现** → 使用锁定的技术栈
5. **验收检查** → 确认所有验收标准通过
6. **提交 PR** → 等待 CI 和 Review
