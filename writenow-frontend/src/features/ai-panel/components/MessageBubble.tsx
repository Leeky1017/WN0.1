    ...
    <div className={cn('flex gap-3 mb-4', isUser ? 'justify-end' : 'justify-start')}>
      ...
      <div className={cn('max-w-[78%] flex flex-col', isUser ? 'items-end' : 'items-start')}>
        {message.skillId && !isUser && (
          <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wide mb-1">
            {message.skillId.split(':').pop()}
          </span>
        )}
        <div
          className={cn(
            'rounded-[var(--radius-md)] px-3 py-2 text-sm whitespace-pre-wrap break-words border',
            isUser
              ? 'bg-[var(--accent)] text-white border-transparent'
              : 'bg-[var(--bg-panel)] text-[var(--text-primary)] border-[var(--border-subtle)]',
          )}
        >
          {message.content}
        </div>
